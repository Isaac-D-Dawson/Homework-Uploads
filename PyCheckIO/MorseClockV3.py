...
# CHANGELOG:
# V 3.0:
# -Replaced multiple call with a for loop
# -Future versions will ideally incude a more complex loop structure
# =This is to avoid the double call currently in use for each digit
# V 2.0:
...
def checkio(time_string: str) -> str:
    ...
    k = [[-2,0], [-2,1], [-3,0], [-3,1], [-3,0], [-3,0]]
    for i in range(0, len(k)):
        outval = encode(round(i/2), k[i][0], k[i][1])
        if i%2 == 0:
            outval = f"{outval}: "
    
    print(outval[0:-6]) #The "[0:-1]" is to trim the trailing ": "
    return(outval[0:-6])
