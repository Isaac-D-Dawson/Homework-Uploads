...
def checkio(first, second):
    ...
    #Check everything in the second input list.
    for i in second.split(","):
        #and if it;s in the first
        if i in first:
            #add it to the output
            outval.append(i)
    
    #This bit sorts the output, then glues it together in string format
    outval.sort()
    for i in outval:
        output = f"{output},{i}"
    
    print(output[1:])#Debug output. the "[1:] is to dodge the extra "," from the gue stage.
    return(output[1:])
